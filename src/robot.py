...
class Robot:
    ...
    def find_duct(self):
        ...
        while(self.stopwatch.time()<300):
            ...
            lowest_ultra_value = min(lowest_ultra_value,self.ultra_front_l.distance(),self.ultra_front_r.distance())
        ...
        if self.motor_l.angle()>escape_angle_l:
            while(abs(self.motor_l.angle())<target_angle_l or abs(self.motor_r.angle())<target_angle_r):
